...
import {Archive,ArrowDown,ArrowUp,ArrowUpDown,CalendarDays,Check,Clapperboard,Clock3,ExternalLink,Film,FolderOpen,Grid2X2,List,LoaderCircle,MapPin,MoreHorizontal,Pencil,Play,Plus,Search,SlidersHorizontal,Trash2,X} from "lucide-react";
...
type VideoLinkField="videoUrl"|"reviewUrl"|"rawFootageUrl";

function VideoTable({entries,canEdit,sortKey,direction,onSort,onSave}:{entries:VideoLogEntry[];canEdit:boolean;sortKey:VideoLogSortKey;direction:VideoLogSortDirection;onSort:(key:VideoLogSortKey)=>void;onSave:(entry:VideoLogEntry)=>Promise<boolean>}){
  const [draft,setDraft]=useState<VideoLogEntry|null>(null);
  const [linkField,setLinkField]=useState<VideoLinkField>("videoUrl");
  const [saving,setSaving]=useState(false);
  const [quickSavingId,setQuickSavingId]=useState<string|null>(null);
  const [rowError,setRowError]=useState("");

  function beginEdit(entry:VideoLogEntry){setDraft({...entry});setLinkField(entry.videoUrl?"videoUrl":entry.reviewUrl?"reviewUrl":"rawFootageUrl");setRowError("")}
  function changeDraft<K extends keyof VideoLogEntry>(key:K,value:VideoLogEntry[K]){setDraft(current=>current?{...current,[key]:value}:current)}
  async function saveDraft(){
    if(!draft)return;
    if(!draft.title.trim()||!draft.client.trim()||!draft.project.trim()||!draft.shootDate||!draft.location.trim()||!draft.producer.trim()){setRowError("Complete the required fields before saving.");return}
    const activeLink=draft[linkField];
    if(activeLink&&!/^https?:\/\//i.test(activeLink)){setRowError("Links must begin with http:// or https://");return}
    setSaving(true);setRowError("");
    const saved=await onSave({...draft,title:draft.title.trim(),client:draft.client.trim(),project:draft.project.trim(),location:draft.location.trim(),locationAddress:draft.locationAddress?.trim()||undefined,producer:draft.producer.trim(),subjects:draft.subjects?.trim()||undefined,updatedAt:new Date().toISOString()});
    setSaving(false);if(saved)setDraft(null);
  }

  async function changeStatus(entry:VideoLogEntry,status:VideoLogStatus){
    setQuickSavingId(entry.id);
    await onSave({...entry,status,updatedAt:new Date().toISOString()});
    setQuickSavingId(null);
  }

  return <div className="video-table-shell"><div className="video-table-hint"><Pencil size={12}/><span>Click the pencil or double-click a row to edit it here.</span></div><table className="video-table" aria-label="Video production archive"><thead><tr><th><SortHeading label="Video" column="title" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th><SortHeading label="Client / project" column="client" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th><SortHeading label="Status" column="status" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th><SortHeading label="Shoot date" column="shootDate" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th><SortHeading label="Location" column="location" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th><SortHeading label="Owner" column="producer" sortKey={sortKey} direction={direction} onSort={onSort}/></th><th>Link</th><th><span className="video-table-actions-label">Actions</span></th></tr></thead><tbody>{entries.map(entry=>{
    if(draft?.id===entry.id)return <tr className="video-inline-row" key={entry.id}><td><div className="video-inline-stack"><input className="video-inline-main" autoFocus value={draft.title} onChange={event=>changeDraft("title",event.target.value)} aria-label="Video title"/><div className="video-inline-split"><select value={draft.videoType} onChange={event=>changeDraft("videoType",event.target.value as VideoLogType)} aria-label="Video type">{videoTypes.map(value=><option key={value}>{value}</option>)}</select><select value={draft.brand} onChange={event=>changeDraft("brand",event.target.value as Brand)} aria-label="Brand"><option value="ARRA">ARRA</option><option value="SQUATCH">SQUATCH</option></select></div>{rowError?<span className="video-inline-error">{rowError}</span>:null}</div></td><td><div className="video-inline-stack"><input value={draft.client} onChange={event=>changeDraft("client",event.target.value)} aria-label="Client"/><input value={draft.project} onChange={event=>changeDraft("project",event.target.value)} aria-label="Project"/></div></td><td><select value={draft.status} onChange={event=>changeDraft("status",event.target.value as VideoLogStatus)} aria-label="Status">{statuses.map(value=><option key={value}>{value}</option>)}</select></td><td><div className="video-inline-stack"><input type="date" value={draft.shootDate} onChange={event=>changeDraft("shootDate",event.target.value)} aria-label="Shoot date"/><input type="number" min="1" max="1440" value={draft.durationMinutes||""} onChange={event=>changeDraft("durationMinutes",event.target.value?Number(event.target.value):undefined)} placeholder="Minutes" aria-label="Duration in minutes"/></div></td><td><div className="video-inline-stack"><input value={draft.location} onChange={event=>changeDraft("location",event.target.value)} aria-label="Location"/><input value={draft.locationAddress||""} onChange={event=>changeDraft("locationAddress",event.target.value||undefined)} placeholder="Address" aria-label="Location address"/></div></td><td><div className="video-inline-stack"><input value={draft.producer} onChange={event=>changeDraft("producer",event.target.value)} aria-label="Owner"/><input value={draft.subjects||""} onChange={event=>changeDraft("subjects",event.target.value||undefined)} placeholder="People / subjects" aria-label="People or subjects"/></div></td><td><div className="video-inline-stack video-inline-link"><select value={linkField} onChange={event=>setLinkField(event.target.value as VideoLinkField)} aria-label="Link type"><option value="videoUrl">Master</option><option value="reviewUrl">Review</option><option value="rawFootageUrl">Files</option></select><input type="url" value={draft[linkField]||""} onChange={event=>changeDraft(linkField,event.target.value||undefined)} placeholder="https://…" aria-label="Video link"/></div></td><td><div className="video-inline-actions"><button className="save" onClick={()=>void saveDraft()} disabled={saving}>{saving?<LoaderCircle className="spin" size={13}/>:<Check size={13}/>}Save</button><button onClick={()=>{setDraft(null);setRowError("")}} disabled={saving}><X size={13}/>Cancel</button></div></td></tr>;
    ...
    return <tr key={entry.id} className={canEdit?"video-table-editable":""} onDoubleClick={event=>{if(canEdit&&!(event.target as HTMLElement).closest("a,button,select,input"))beginEdit(entry)}}><td><div className="video-table-title"><span className={`video-table-mark ${entry.brand.toLowerCase()}`}><Play size={12} fill="currentColor"/></span><span><b>{entry.title}</b><small>{entry.videoType} · {entry.brand}</small></span>{canEdit?<button className="video-title-edit" onClick={()=>beginEdit(entry)} aria-label={`Edit ${entry.title} inline`} title="Edit inline"><Pencil size={13}/></button>:null}</div></td><td><b>{entry.client}</b><small>{entry.project}</small></td><td>{canEdit?<div className="video-status-quick"><select value={entry.status} onChange={event=>void changeStatus(entry,event.target.value as VideoLogStatus)} disabled={quickSavingId===entry.id} aria-label={`Change status for ${entry.title}`}>{statuses.map(value=><option key={value}>{value}</option>)}</select>{quickSavingId===entry.id?<LoaderCircle className="spin" size={12}/>:null}</div>:<span className={`video-table-status ${entry.status.toLowerCase().replaceAll(" ","-")}`}>{entry.status}</span>}</td><td><b>{displayDate(entry.shootDate)}</b><small>{entry.durationMinutes?`${entry.durationMinutes} min`:"Duration open"}</small></td><td><b>{entry.location}</b>{entry.locationAddress?<small>{entry.locationAddress}</small>:null}</td><td><b>{entry.producer}</b><small>{entry.subjects||"Production owner"}</small></td><td>{link?<a className="video-table-link" href={link} target="_blank" rel="noreferrer">{linkLabel}<ExternalLink size={12}/></a>:<span className="video-table-no-link">No link</span>}</td><td>{canEdit?<button className="video-table-edit" onClick={()=>beginEdit(entry)} aria-label={`Edit ${entry.title} inline`} title="Edit inline"><MoreHorizontal size={18}/></button>:null}</td></tr>;
  })}</tbody></table></div>;
}
...
export function VideoLogPage({brand,allowedProjects,query,onQueryChange}:{brand:"ALL"|Brand;allowedProjects:Set<string>|null;query:string;onQueryChange:(value:string)=>void}){
  ...
  async function save(entry:VideoLogEntry,isNew:boolean):Promise<boolean>{
    setEditing(null);setError("");
    setEntries(current=>isNew?[entry,...current]:current.map(item=>item.id===entry.id?entry:item));
    try{if(!supabase)throw new Error("Supabase is not configured.");await upsertVideoLog(supabase,entry,isNew?profile.id:undefined);return true}
    catch(saveError){console.error("[video-log] save failed",saveError);setError("That video entry couldn’t be saved. The shared archive has been restored.");try{await refresh()}catch(refreshError){console.error("[video-log] recovery failed",refreshError)}return false}
  }
  ...
  return <div className="content video-log-page"><div className="page-head"><div><span>PRODUCTION LIBRARY</span><h1>Video Log</h1><p>A searchable archive of every shoot, edit, review, and published video.</p></div>{canEdit?<button className="primary" onClick={()=>setEditing("new")}><Plus size={15}/>Log video</button>:null}</div>
    ...
    {loading?<div className="video-empty"><Clapperboard size={27}/><h2>Opening the video archive…</h2><p>Syncing shared production records.</p></div>:visible.length?(view==="list"?<VideoTable entries={sortedVisible} canEdit={canEdit} sortKey={sortKey} direction={sortDirection} onSort={changeSort} onSave={entry=>save(entry,false)}/>:<div className="video-grid">{sortedVisible.map(entry=><VideoCard key={entry.id} entry={entry} canEdit={canEdit} onEdit={setEditing}/>)}</div>):<div className="video-empty"><Clapperboard size={27}/><h2>{scoped.length?"No videos match these filters":"Start your production archive"}</h2><p>{scoped.length?"Try another search, status, or video type.":"Log shoots, edits, client reviews, and finished videos in one place."}</p>{canEdit&&!scoped.length?<button className="primary" onClick={()=>setEditing("new")}><Plus size={15}/>Log your first video</button>:null}</div>}
